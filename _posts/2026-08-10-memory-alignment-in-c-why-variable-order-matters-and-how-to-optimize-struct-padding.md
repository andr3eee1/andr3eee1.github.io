...
When programmers first learn C, memory is often introduced as a continuous linear array of bytes, indexed from $0$ to $2^{64}-1$ on modern 64-bit systems. While this model is conceptually helpful, modern CPU hardware does not access memory byte-by-byte. Instead, memory architectures operate on word-sized granularity—typically 32-bit (4-byte), 64-bit (8-byte), or even 64-byte cache line chunks.
...
In this deep-dive article, we will examine the hardware mechanics behind memory alignment, unpack compiler struct padding and tail padding rules, construct visual byte-level layout models, and explore actionable optimization strategies.
...
What happens when an 8-byte `double` is placed at address `0x1004` (not divisible by 8)?
...
| `long` (64-bit Linux), `double`, `uint64_t`, pointers (`void*`) | 8 bytes | 8 bytes (Divisible by 8) |
| `long double` (x86 extended) | 16 bytes | 16 bytes (Divisible by 16) |
...
- **Offset `0x00` (`a`):** `char` requires 1-byte alignment. Occupies byte `0`. Next free offset: `0x01`.
...
- **Offset `0x08` to `0x0F` (`b`):** `double` occupies bytes `8` to `15`. Next free offset: `0x10` (16).
- **Offset `0x10` to `0x13` (`c`):** `int` requires 4-byte alignment. `16` is divisible by 4 ($16 \pmod 4 = 0$). Fits immediately! Occupies bytes `16` to `19`. Next free offset: `0x14` (20).
- **Offset `0x14` to `0x15` (`d`):** `short` requires 2-byte alignment. `20` is divisible by 2. Occupies bytes `20` and `21`. Next free offset: `0x16` (22).
- **Offset `0x16` (`e`):** `char` requires 1-byte alignment. Occupies byte `22`. Next free offset: `0x17` (23).
...
We can minimize or eliminate struct padding by applying a simple rule:
...
- **Offset `0x00` to `0x07` (`b`):** `double` starting at offset `0`. Next free: `0x08`.
- **Offset `0x08` to `0x0B` (`c`):** `int` requires 4-byte alignment. `8` is divisible by 4. Fits at offset `8` with **0 padding bytes**! Next free: `0x0C` (12).
- **Offset `0x0C` to `0x0D` (`d`):** `short` requires 2-byte alignment. `12` is divisible by 2. Fits at offset `12` with **0 padding bytes**! Next free: `0x0E` (14).
- **Offset `0x0E` (`a`):** `char` placed at offset `14`. Next free: `0x0F` (15).
- **Offset `0x0F` (`e`):** `char` placed at offset `15`. Next free: `0x10` (16).
- **Tail Padding Check:** Total bytes used = 16. $\text{Align}_{\max} = 8$. Is 16 divisible by 8? Yes ($16 \pmod 8 = 0$). **0 tail padding bytes required.**
...
By simply reordering variables, we save **80 MB of RAM** and significantly improve CPU L1/L2 cache line hits!
...
Save and compile the following standard C program:
...
### Execution Output (GCC / Clang x86_64)

=== UNOPTIMIZED STRUCT ===
Total Size: 24 bytes
Offset of a: 0
Offset of b: 8 (Padding before b: 7)
Offset of c: 16
Offset of d: 20
Offset of e: 22
...
- **Pointer Violations:** Taking the address of an unaligned member (`int *ptr = &packed_inst.payload;`) causes undefined behavior in standard C.

**When to use packing:** Only when mapping external binary streams (e.g., parsing TCP/IP packets or ELF headers) where protocol specifications require exact layout adherence.
...
When two threads running on separate CPU cores concurrently modify two distinct variables located on the **same cache line**, the CPU cache coherency protocol (e.g., MESI protocol) repeatedly invalidates and re-fetches the cache line across cores. This bottleneck is called **false sharing**.
...
int main(void) {
    size_t alignment = 64; // Must be a valid alignment supported by implementation
    size_t size = 1024;    // Must be a multiple of alignment
...
*Note:* For POSIX platforms, `posix_memalign()` provides an alternative interface:
...
By keeping these rules in mind, you will produce clean, high-performance C code that works harmoniously with modern CPU memory architectures.
